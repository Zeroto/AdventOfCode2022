let input = "CjhshBJCSrTTsLwqwqwb
...
let findDuplicate (bag: char[] seq) =
  let c1 = Set.ofArray (bag |> Seq.head)
  let c2 = Set.ofArray (bag |> Seq.item 1)

  Set.intersect c1 c2 |> Set.toSeq |> Seq.head

let part1 =
  bags
  |> Array.map (findDuplicate >> charToNumber)
  |> Array.sum
